...
export default function CataloguePage() {
  ...
  useEffect(() => {
    ...
    loadMetadataForFolder(session.metadataLocation, selectedBrowseLocation)
      ...
      .catch((err) => {
        if (!ignore) {
          setLoadError(err instanceof Error ? err.message : 'Failed to read your catalogue.')
          setIsLoadingMetadata(false)
        }
      })
    return () => {
  ...
  async function handleSaveTags(entry: JottaEntry, loc: MountpointRef, tags: Record<string, string[]>) {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : 'Failed to save to your catalogue.'
  ...
  if (!session.metadataLocation) {
    return (
      <div className="mx-auto flex w-full max-w-lg flex-1 flex-col gap-4 px-6 py-10">
        <div>
          <h1 className="text-2xl font-semibold">Set up your catalogue</h1>
    ...
    return (
      <div className="mx-auto flex w-full max-w-lg flex-1 flex-col items-center justify-center gap-4 px-6 text-center">
        ...
        <p className="text-sm text-zinc-500">
          Nothing has been lost — your catalogue is stored in Jottacloud, and this only failed to
          read it.
        </p>
  ...
  // Step 2 — that folder's part of the catalogue is on its way.
  if (!store) {
    return (
      <div className="mx-auto flex w-full max-w-2xl flex-1 flex-col gap-6 px-6 py-10">
        {header}
        <div className="flex items-center gap-2 text-sm text-zinc-500">
          ...
          Reading your catalogue for {folderLabel}…
        </div>
      ...
      <div className="flex gap-2 border-b border-zinc-200 pb-2 dark:border-zinc-800">
        {(
          [
            ['assign', 'Describe'],
            ['browse', 'Find'],
            ['batch', 'Describe in bulk'],
            ['classify', 'AI suggestions'],
